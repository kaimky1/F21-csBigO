...
// ANSWER: numbers[3] but the time complexity will stay the same. 
...
// ANSWER: I believe it is possible by doing the splice method instead of pushing to a new array. 
function getNumbersSquared(numbers) {
  let j = 0;
  for (let i = 0; i < numbers.length; i++) {
    let squaredValue = numbers[i] * numbers[i];
    answers.splice(j, 0, squaredValue);
    j++;
  }
  return answers
}
...
// ANSWER: The space complexity of countLetters and countLettersRounTwo can grow linearly dependening on the string size. 
